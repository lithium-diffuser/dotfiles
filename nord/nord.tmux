...
NORD_TMUX_STATUS_CONTENT_NO_PATCHED_FONT_FILE
...
__load() {
  ...
  local status_content=$(tmux show-option -gqv 
"$NORD_TMUX_STATUS_CONTENT_OPTION")
  local no_patched_font=$(tmux show-option -gqv 
"$NORD_TMUX_NO_PATCHED_FONT_OPTION")
  local date_format=$(tmux show-option -gqv 
"$NORD_TMUX_STATUS_CONTENT_DATE_FORMAT")
  ...
  if [ "$status_content" != "0" ]; then
    if [ "$no_patched_font" != "1" ]; then
      ...
      tmux source-file 
"$_current_dir/$NORD_TMUX_STATUS_CONTENT_NO_PATCHED_FONT_FILE"
    fi
  fi
}
